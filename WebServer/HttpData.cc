...
void HttpData::handleRead()
{
    ...
    do
    {
        ...
        if(m_state == STATE_RECV_BODY)
        {
            ...
            if(static_cast<int>(m_inBuffer.size()) < content_length) break;
            m_state = STATE_ANALYSIS;
        }
    ...
    if(!m_error)
    {
        ...
        if (!m_error && m_state == STATE_FINISH)
        {
            ...
            {
                if (m_connectionState != H_DISCONNECTING) handleRead();
            }
        }
...
void HttpData::handleWrite()
{
    ...
    {
        __uint32_t& events = m_channel->getEvents();
        if (writen(m_fd, m_outBuffer) < 0)
        {
            perror("writen");
            events = 0;
            m_error = true;
        }
        if (m_outBuffer.size() > 0) events |= EPOLLOUT;
    }
}
...
void HttpData::handleConn()
{
    ...
    else if (!m_error && m_connectionState == H_DISCONNECTING && (events & EPOLLOUT))
    {
...
URIState HttpData::parseURI()
{
    ...
    else 
    {
        ...
        else 
        {
            ...
            if (ver == "1.0") m_HTTPVersion = HTTP_10;
            else if (ver == "1.1") m_HTTPVersion = HTTP_11;
            else return PARSE_URI_ERROR;
        }
    }
    return PARSE_URI_SUCCESS;
...
AnalysisState HttpData::analysisRequest() 
{
    if (m_method == METHOD_POST) 
    {
        ...
        // }
        // int length = stoi(m_headers["Content-length"]);
        ...
        // "\r\n\r\n";
        // m_outBuffer += header + string(data_encode.begin(), data_encode.end());
    ...
    else if (m_method == METHOD_GET || m_method == METHOD_HEAD) 
    {
        ...
        if (m_fileName == "favicon.ico") 
        {
            ...
            header += "\r\n";
            m_outBuffer += header;
            m_outBuffer += string(favicon, favicon + sizeof favicon);
            return ANALYSIS_SUCCESS;
        }
        ...
        if (m_method == METHOD_HEAD) return ANALYSIS_SUCCESS;
        ...
        m_outBuffer += string(src_addr, src_addr + sbuf.st_size);
        munmap(mmapRet, sbuf.st_size);
        return ANALYSIS_SUCCESS;
    }
    return ANALYSIS_ERROR;
